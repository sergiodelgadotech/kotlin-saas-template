...
- Each task goes on a **feature branch** → commit → push → open a PR (title must be a [conventional commit](https://www.conventionalcommits.org/)) → CI must pass and branch must be up to date with `main` → squash-merge. Do not push directly to `main` — branch protection enforces this.
...
Social login buttons (Google, GitHub, Microsoft, Apple) appear automatically when the matching `ZITADEL_DEV_*` credentials are set in `.env` (loaded by direnv, forwarded to `zitadel-init`). Each provider is optional and silently skipped when its env vars are absent, so the stack always boots cleanly. To activate a provider, register your OAuth app in the relevant developer console with callback URL `http://localhost:3000/ui/v2/login/idps/callback` (port 3000 — the login-v2 UI, not Zitadel core on 8089), fill in the vars documented in `.env.example`, then re-seed:
...
**Important:** Don't run `sbx rm` until the PR for that sandbox's branch has been merged — removing the sandbox deletes the branch.
...
**`bootRun` inside the sandbox (fixed):** `bootRun` used to fail in an sbx microVM because the two `network_mode: host` Zitadel helper containers can't share the microVM netns: `zitadel-init` got **connection refused** on `localhost:8089` and exited 1 (aborting Spring Boot's docker-compose lifecycle), and `zitadel-login`'s healthcheck couldn't even resolve `localhost`. Both are now fixed so the same `docker-compose.yml` boots on plain Docker/Podman *and* inside sbx:

- **`zitadel-init`** no longer uses host networking. It connects over the Docker bridge (`ZITADEL_CONNECT_URL=http://zitadel:8080`) while `init.py` sends an explicit `Host: localhost:8089` header on every request, so Zitadel's vhost-based OIDC routing still resolves the instance (bare `zitadel:8080` Host returns 404). The JWT audience stays the external URL (`ZITADEL_URL=http://localhost:8089`). This is documented fix (a) from the old note.
- **`zitadel-login`** also no longer uses host networking — it runs on the bridge and reaches Zitadel through a small **`zitadel-proxy`** (nginx) sidecar that rewrites the request `Host` to `localhost:8089` (`docker/zitadel-proxy/nginx.conf`). Zitadel matches the instance by Host *domain* (ignoring the port), but echoes the full Host into generated URLs, so the proxy keeps the `:8089` authority to keep the issuer/endpoints correct (`http://localhost:8089`). Login's port `3000` is published explicitly (it was implicitly host-exposed before), and its healthcheck is overridden to probe the **liveness** endpoint via the numeric loopback (`http://127.0.0.1:3000/ui/v2/login/healthy`). This is the unified version of old fixes (a)+(b).

The full browser OIDC flow works in sbx now: `localhost:8080` → Zitadel `authorize` (`:8089`) → login UI (`:3000`) resolves the auth request via the proxy and renders the login form (HTTP 200). To use it from the host browser, publish the ports: `sbx ports <name> --publish 8080:8080/tcp --publish 8089:8089/tcp --publish 3000:3000/tcp`. The same `docker-compose.yml` still works on plain Docker/Podman (the proxy is environment-agnostic), but re-verify the host login flow there since the host setup changed from `network_mode: host` to bridge+proxy.
...
Include `Closes #N` in the PR body to auto-close the plan issue when the PR is merged.
