...
- **`zitadel-login`** keeps `network_mode: host` (so the host/Podman login flow is unchanged), but its healthcheck is overridden in `docker-compose.yml` to probe the **liveness** endpoint via the numeric loopback (`http://127.0.0.1:3000/ui/v2/login/healthy`) instead of the image default (`localhost` + readiness), which never goes healthy in sbx.

Residual sbx caveat: because `zitadel-login` still relies on host networking, inside sbx it can't actually reach Zitadel, so the browser OIDC login flow won't complete there — the app boots and most surfaces are exercisable, but full SSO needs the host/Podman setup. Fully fixing login-in-sbx would mean moving it onto the bridge behind a Host-rewriting proxy (old fix (b)); deferred to avoid regressing the primary host login flow.
